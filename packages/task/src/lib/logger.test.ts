...
test('logger meta is a closed event schema at compile time', () => {
  ...
  logger.log({ label: 'unknown-label', body: 'x', meta: { command: 'migrate' } })

  // @ts-expect-error 既知 label でも schema に無い field は渡せない
  logger.log({ label: 'task', body: 'x', meta: { command: 'migrate', secret: 'x' } })

  // @ts-expect-error meta の値は primitive のみで nested object は渡せない
  logger.log({ label: 'task', body: 'x', meta: { command: { name: 'migrate' } } })

  const payload: Record<string, unknown> = { command: 'migrate' }
  // @ts-expect-error 型の広い payload 変数を meta へ渡せない
  logger.log({ label: 'task', body: 'x', meta: payload })
